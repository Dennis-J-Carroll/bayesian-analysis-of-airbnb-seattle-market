...
def create_neighborhood_comparison_plot(df, metric='price_clean', top_n=15):
    ...
    neighborhood_stats = df.groupby('neighbourhood_cleansed')[metric].agg(['mean', 'median', 'count']).reset_index()
    neighborhood_stats = neighborhood_stats.sort_values('mean', ascending=False).head(top_n)
...
def create_scatter_matrix(df):
    ...
    plot_df = df[['price_clean', 'accommodates', 'number_of_reviews',
                  'review_scores_rating', 'availability_365']].dropna()
...
def create_room_type_analysis(df):
    ...
    room_stats = df.groupby('room_type')['price_clean'].agg(['mean', 'median', 'count']).reset_index()
    ...
    # Bar chart
    fig.add_trace(
        go.Bar(x=room_stats['room_type'], y=room_stats['mean'],
    ...
    # Pie chart
    fig.add_trace(
        go.Pie(labels=room_stats['room_type'], values=room_stats['count'],
...
def create_availability_price_plot(df):
    ...
    df_plot = df.copy()
    df_plot['availability_bin'] = pd.cut(df_plot['availability_365'],
                                         bins=[0, 90, 180, 270, 365],
                                         labels=['0-90 days', '90-180 days',
                                                '180-270 days', '270-365 days'])
...
def create_review_impact_plot(df):
    ...
    df_plot = df.copy()
    df_plot['review_category'] = pd.cut(df_plot['number_of_reviews'],
                                        bins=[-1, 0, 10, 50, 500],
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🏠 Seattle Airbnb Price Intelligence Dashboard</h1>',
                unsafe_allow_html=True)
    ...
    page = st.sidebar.radio(
        "Select Page",
        ["🏠 Overview", "🔍 Neighborhood Analysis", "💰 Price Prediction",
         "📈 Market Intelligence", "🎯 Business Strategy", "🔬 Model Insights"]
    )
    ...
    neighborhoods = ['All Neighborhoods'] + sorted(df['neighbourhood_cleansed'].unique().tolist())
    ...
    if selected_neighborhood != "All Neighborhoods":
        df_filtered = df_filtered[df_filtered['neighbourhood_cleansed'] == selected_neighborhood]
...
def show_overview_page(df_filtered, df_full):
    ...
    with col1:
        st.plotly_chart(create_price_distribution_plot(df_filtered), use_container_width=True)
        st.plotly_chart(create_room_type_analysis(df_filtered), use_container_width=True)

    with col2:
        st.plotly_chart(create_neighborhood_comparison_plot(df_filtered, top_n=10), use_container_width=True)
        st.plotly_chart(create_availability_price_plot(df_filtered), use_container_width=True)

    # Insights
    st.markdown('<div class="insight-box">', unsafe_allow_html=True)
    st.markdown("#### 💡 Key Insights")

    most_expensive = df_filtered.groupby('neighbourhood_cleansed')['price_clean'].mean().idxmax()
    most_expensive_price = df_filtered.groupby('neighbourhood_cleansed')['price_clean'].mean().max()
...
def show_neighborhood_page(df_filtered, df_full):
    ...
    with col4:
        price_vs_city = ((df_neighborhood['price_clean'].mean() / df_full['price_clean'].mean()) - 1) * 100
    ...
    with col1:
        st.plotly_chart(create_price_distribution_plot(df_full, neighborhood), use_container_width=True)
        st.plotly_chart(create_room_type_analysis(df_neighborhood), use_container_width=True)

    with col2:
        st.plotly_chart(create_review_impact_plot(df_neighborhood), use_container_width=True)

        # Accommodates analysis
        acc_stats = df_neighborhood.groupby('accommodates')['price_clean'].agg(['mean', 'count']).reset_index()
        acc_stats = acc_stats[acc_stats['count'] >= 5]  # Filter sparse data

        fig = px.line(acc_stats, x='accommodates', y='mean', markers=True,
    ...
    stats_df = pd.DataFrame({
        'Metric': ['Mean Price', 'Median Price', 'Std Dev', 'Min Price', 'Max Price',
                  'Mean Accommodates', 'Avg Reviews', 'Avg Rating', 'Avg Availability'],
        'Value': [
...
def show_prediction_page(df):
    ...
    st.info("🔮 **Note**: This is a demonstration interface. Full Bayesian predictions require the trained model.")

    # Input form
    col1, col2, col3 = st.columns(3)

    with col1:
        neighborhood = st.selectbox("Neighborhood", sorted(df['neighbourhood_cleansed'].unique()))
    ...
    if st.button("🎯 Generate Price Prediction", type="primary"):
        with st.spinner("Running Bayesian inference..."):
            ...
            if len(similar_listings) > 0:
                ...
                with col2:
                    st.metric("80% Confidence Interval",
                ...
                # Visualization
                x = np.linspace(max(0, mean_price - 3*std_price), mean_price + 3*std_price, 300)
                y = stats.norm.pdf(x, mean_price, std_price)

                fig = go.Figure()

                fig.add_trace(go.Scatter(x=x, y=y, fill='tozeroy', name='Posterior Distribution',
                                        fillcolor='rgba(70, 130, 180, 0.3)', line=dict(color='steelblue')))

                fig.add_vline(x=mean_price, line_dash="dash", line_color="red",
                             annotation_text=f"Mean: ${mean_price:.0f}")
                fig.add_vrect(x0=ci_95_lower, x1=ci_95_upper, fillcolor="green", opacity=0.1,
                ...
                st.warning("⚠️ Not enough similar listings for accurate prediction. Try adjusting parameters.")
...
def show_market_intelligence_page(df_filtered, df_full):
    ...
    with col1:
        # Supply analysis
        supply_by_neighborhood = df_full.groupby('neighbourhood_cleansed').size().sort_values(ascending=False).head(15)
    ...
    with col2:
        # Price vs Supply
        neighborhood_metrics = df_full.groupby('neighbourhood_cleansed').agg({
            'price_clean': 'mean',
            'id': 'count'
        }).reset_index()
    ...
    with col1:
        # Price volatility
        price_volatility = df_full.groupby('neighbourhood_cleansed')['price_clean'].std().sort_values(ascending=False).head(10)
    ...
    with col2:
        # Correlation heatmap
        corr_vars = df_full[['price_clean', 'accommodates', 'number_of_reviews',
                            'review_scores_rating', 'availability_365']].corr()
    ...
    with col3:
        # Review distribution
        review_bins = pd.cut(df_full['number_of_reviews'], bins=[-1, 0, 10, 50, 100, 500],
...
def show_business_strategy_page(df):
    ...
    **Strategic Investment Intelligence**: Identify optimal neighborhoods and property types for maximum ROI.
    ...
    with col1:
        investment = st.number_input("Investment Amount ($)", 50000, 5000000, 200000, 50000)
    with col2:
    ...
    neighborhood_roi = df.groupby('neighbourhood_cleansed').agg({
        'price_clean': 'mean',
        'availability_365': 'mean',
        'id': 'count'
    }).reset_index()

    neighborhood_roi.columns = ['Neighborhood', 'Avg Price', 'Avg Availability', 'Supply']

    # Calculate annual revenue
    neighborhood_roi['Est Annual Revenue'] = (
        neighborhood_roi['Avg Price'] * 365 * occupancy * (1 - neighborhood_roi['Avg Availability'] / 365)
    )

    # Calculate ROI
    neighborhood_roi['Est ROI %'] = (
        (neighborhood_roi['Est Annual Revenue'] * (1 - operating_cost)) / investment * 100
    )
    ...
    with col1:
        ...
        fig = go.Figure(go.Bar(
            ...
            orientation='h',
            marker_color=np.where(neighborhood_roi.head(15)['Est ROI %'] >= target_roi, 'green', 'orange')
        ))

        fig.add_vline(x=target_roi, line_dash="dash", line_color="red",
    ...
    with col2:
        ...
        fig.add_hline(y=target_roi, line_dash="dash", line_color="red",
    ...
    top_opportunities = neighborhood_roi.head(10)[['Neighborhood', 'Avg Price', 'Supply',
                                                    'Est Annual Revenue', 'Est ROI %']]

    # Format the dataframe
    top_opportunities['Avg Price'] = top_opportunities['Avg Price'].apply(lambda x: f"${x:.0f}")
    top_opportunities['Est Annual Revenue'] = top_opportunities['Est Annual Revenue'].apply(lambda x: f"${x:,.0f}")
    top_opportunities['Est ROI %'] = top_opportunities['Est ROI %'].apply(lambda x: f"{x:.1f}%")
...
def show_model_insights_page():
    ...
    feature_importance = pd.DataFrame({
        'Feature': ['Accommodates', 'Entire Home', 'Review Score', 'Reviews Count',
